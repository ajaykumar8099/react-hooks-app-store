...
const AppStore = () => {
  ...
  const [tabDetails, setTabDetails] = useState(tabsList[0].tabId)
  const filterTabs = appsList.filter(each => each.category === tabDetails)

  const onClickTab = tabId => {
    const resultsTabItem = tabsList.filter(each => each.tabId === tabId)
    setTabDetails(resultsTabItem[0].tabId)
  }
  ...
  const searchList = filterTabs.filter(each =>
        ...
        <ul className="ul-list-tabs">
          {tabsList.map(each => (
            <TabItem
              ...
              isActive={each.tabId === tabDetails}
            />
          ))}
        </ul>
        <ul className="app-ul-li-cont">
          {searchList.map(each => (
            <AppItem key={each.appId} data={each} />
          ))}
        </ul>
      </div>
    </div>
